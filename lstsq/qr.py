...
class QR(object):
    ...
    - b: a column vector of shape (M,). Can also be multiple column
      ...
      (M, K).
    - reduce: bool indicating whether to return
    ...
    def gram_schmidt(self):
        ...
        for i in range(N):
            ...
            for j in range(i):
                ...
                p = projection(self.A[:, i], self.A[:, j])
                self.A[:, i] -= p
            # normalize ith column
            self.A[:, i] /= l2_norm(self.A[:, i])
    ...
    def solve(self, b):
        """
        Perform the QR factorization of the matrix A,
        then solve the linear system Ax = b using
